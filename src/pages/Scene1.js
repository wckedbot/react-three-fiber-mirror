...
import {ThinFilmFresnelMap} from '../components/Thinfilmfresnelmap/ThinFilmFresnelMap';
...
function Title({layers, ...props}) {
    ...
    return (
        <group {...props} ref={group}>
            <Text ref={textRef} name="Shreyansh" depthTest={false} material-tonMapped={false} material-color="#FFFFFF" {...text}>
               Prajna
            </Text>
        </group>
    )
...
function Mirror({sideMaterial, reflectionMaterial, args, layers, ...props}) {
    ...
    return(
        <Box {...props} ref={ref} args={args} material={[sideMaterial, sideMaterial, sideMaterial, sideMaterial, reflectionMaterial, reflectionMaterial]} />
    )
}
      ...
      <>
        <group name="sceneContainer" ref={group}>
          <Octahedron layers={[11]} name="background" args={[20, 4, 4]} position={[0, 0, -5]}>
            <meshMatcapMaterial matcap={matcapTexture} side={THREE.BackSide} transparent opacity={0.7} color="#FFFFFF" />
          </Octahedron>
          <cubeCamera
            layers={[11]}
            name="cubeCamera"
